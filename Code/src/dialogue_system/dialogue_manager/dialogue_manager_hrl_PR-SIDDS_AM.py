...
class DialogueManager_HRL(object):
    ...
    def __init__(self, user, agent, parameter):
        ...
        self.dis_sym = set(self.slot_set)-fslot_set
        self.GA = pickle.load(open("MD/GA.p", 'rb'))
        ###SymptomAssociationMatrix
        #self.SA = pickle.load(open("SS.p", 'rb'))
        #self.SDA = pickle.load(open("SD.p", 'rb'))
        #self.SA_ = pickle.load(open("SD+.p", 'rb'))
        self.SA_ = pickle.load(open("MD/SD++.p", 'rb'))
        self.SID = pickle.load(open("MD/SID.p", 'rb'))
        self.IDS = pickle.load(open("MD/IDS.p", 'rb'))
        self.DID = pickle.load(open("MD/DID.p", 'rb'))
        self.IDD = pickle.load(open("MD/IDD.p", 'rb'))
        ...
        #     path = 'Data/label'+str(i) + '/slot_set.p' #For Testing
        ...
        ##There is a diff bw DID and disease2id
    ...
    def next(self, greedy_strategy, save_record, index):
        ...
        csr = copy.deepcopy(state["current_slots"]["inform_slots"])
        # t = {}
        ...
        # csrr = t
        tt = {}
        tt[list(self.state_tracker.user.goal["goal"]["explicit_inform_slots"].keys())[0]] = True
        #self.pr = self.recommend(csr, tt)
        if self.parameter.get("agent_id") == 'agentdqn':
        ...
        cs = copy.deepcopy(state["current_slots"]["inform_slots"])
        t = {}
        ...
        if self.parameter.get("disease_as_action")==False:
            ...
            if action_type == "disease" or condition:# or lower_action in self.lower_action_history:
                ...
                if self.parameter.get("train_mode") == False:
                    arr = Ys.detach().cpu().numpy()
                    #top_2_indices = list(np.argpartition(arr, -2, axis=1)[0][-2:])
                    #top_3_indices = list(np.argpartition(arr, -3, axis=1)[0][-3:])
                    #top_4_indices = list(np.argpartition(arr, -4, axis=1)[0][-4:])
                    top_5_indices = list(np.argpartition(arr, -5, axis=1)[0][-5:])
                    #disease_record = [disease] + [self.id2disease[i] for i in top_2_indices]
                    #disease_record = [disease] + [self.id2disease[i] for i in top_3_indices]
                    #disease_record = [disease] + [self.id2disease[i] for i in top_3_indices]
                    disease_record = [disease] + [self.id2disease[i] for i in top_5_indices]

                    self.disease_record.append(disease_record)



                print('## Predicted Diease : ',self.id2disease[pre_disease[0]])
                self.disease_replay.append((state_rep, self.disease2id[disease]))
            ...
            #         self.disease_record.append([disease,self.id2disease[pre_disease[0]]])
                ...
                # dsr = self.disease_symptom_association(css,self.id2disease[pre_disease[0]])
                # if dsr > 0.4:
                #     DS_R = dsr *15* gamma
                # else:
                #     DS_R = -10*(1-dsr)
                # print('Diease Similarity Reward: ',DS_R)
        ...
        # User takes action.
        #css = copy.deepcopy(state["current_slots"]["inform_slots"])
        rr = 0
        gr = 0
        if master_action_index!=9:

            ar = self.association(tt, agent_action["request_slots"])
            self.state_tracker.asc(ar)
            x, y = self.relevance_Score(self.state_tracker.user.goal["goal"]["explicit_inform_slots"],css, agent_action["request_slots"])
            self.rsp = self.rsp + x
            self.rsc = self.rsc + y
            current_r=list(agent_action["request_slots"].keys())[0]
            # if len(set(self.pr).intersection(set(self.GA[self.label[master_action_index]])))==0: #Recommeded symp not in group, so group might be wrong.
            # #     gr = 2*gamma
            #     gr = -4*gamma
            #     print('Recommeded symptom is out of group')
            # if current_r in self.pr:
            #     print('Recommended and asked the same')
            #     rr = +15
            # # elif len(set(self.pr).intersection(set(self.GA[self.label[master_action_index]])))!=0: #Recommeded symp not in group, so group might be wrong.
            # #     gr = 2*gamma
            # #     rr = 2*gamma
            # #     print('Recommeded symptom is out of group')
            # else:
            #     print('Penalized for not asking from recommended symptoms')
            #     rr = -2


            if ar>=0.8:
                ar = +15
            elif ar>0.2 and ar<0.8:
                ar = +1
            elif ar<0.2:
                ar = -2
        ...
        if self.parameter.get("agent_id") == "agenthrljoint2" and self.parameter.get("disease_as_action")==False and agent_action["action"]!="inform":
            ...
            reward, lower_reward = self.next_by_hrl_joint2(dialogue_status, lower_action, state, master_action_index, group_id, episode_over, reward,ar)
            #lower_reward = lower_reward +(ar + rr)
        ...
        #print(reward)
        #fr = self.FetauredReward(master_action_index,agent_action)

        # if(master_action_index!=9):
        #     csr = copy.deepcopy(state["current_slots"]["inform_slots"])
        #     print('Complete Inform: ',csr)
        #     t = {}
        #     for key,value in csr.items():
        #         if value==True:
        #             t[key]=True
        #     csrr = t
        #     self.pr = self.recommend(csrr,agent_action['request_slots'])

        reward = reward + gr
        #RS = [fr,ar,rr,DS_R]
        #self.state_tracker.RewardUpdate(RS)
        ##Range of ar : [0,0.883] and dsr : [0,0.779]
        if save_record is True:
            if self.parameter.get('prioritized_replay'):
                ...
                targetrecommend_action_value = self.state_tracker.agent.next_state_values_DDQN(state)
        ...
        return reward, episode_over, dialogue_status, slots_proportion_list,RS

    def association(self,informs, current_request):

        current_request = list(current_request.keys())[0]
        informs = list(informs.keys())
        sas = 0
        rid = self.SID[current_request]
        for i in range(0,len(informs)):
            tid = self.SID[informs[i]]
            ass = self.SA_[tid][rid]
            sas = sas + ass
        return sas

    # def recommend(self,all_inform, current_informs):
    #     print('Inside Recommendation Block')
    #     all_infokey = list(all_inform.keys())
    #     sac = [0]*len(self.slot_set)
    #     slist = list(self.slot_set)
    #     for i in range(0,len(self.slot_set)):
    #         iid = self.SID[slist[i]]
    #         temp =0
    #         for j in current_informs:
    #             jid = self.SID[j]
    #             t = self.SA_[jid][iid]
    #             temp = temp + t
    #         sac[i] = temp
    #     rs4 = [0]*len(self.slot_set)
    #     AL = sac
    #     ALD = dict(enumerate(AL))
    #     SALD = dict(sorted(ALD.items(), key=lambda x: x[1]))
    #     KK = list(SALD.keys())
    #     ##Top 2
    #     ind = []
    #     i = 1
    #     j = 1
    #     slt = list(self.slot_set.keys())
    #     while(i<3):
    #         p = -j
    #         print('checking symptom:',slt[KK[p]])
    #         if slt[KK[p]] not in all_infokey:
    #             rs4[KK[p]] = 1
    #             ind = ind + [KK[p]]
    #             i = i+1
    #             j = j+1
    ...
    #
    #
    #     RL = []   ##Recomendation list
    #     for j in ind:
    #         temp = 'S_' + str(j)
    #         RL = RL + [self.IDS[temp]]
    #
    #     print('Recommendation List:',RL)
    #
    #     self.state_tracker.rs(rs4)
    #     return RL

    def relevance_Score(self,patient_report,informs, req_Sym):
        spr = 0
        scc = 0
        current_request = list(req_Sym.keys())[0]
        informs = list(informs.keys())
        rid = self.SID[current_request]
        for i in range(0,len(informs)):
            tid = self.SID[informs[i]]
            ass = self.SA_[tid][rid]
            scc = scc + ass
        patient_report = list(patient_report.keys())
        for i in range(0,len(patient_report)):
            tid = self.SID[patient_report[i]]
            asr = self.SA_[tid][rid]
            spr = spr + asr
        return spr, scc

    def disease_symptom_association(self,inform,predicted_diease):
        id = self.DID[predicted_diease]
        ss = 0
        for i in inform:
            t = self.SID[i]
            ss = ss + self.SA_[t][id]
    ...
    # def FetauredReward(self,master_action_index,agent_action):
    #     f_reward = 0
    #     if master_action_index!=9:
    #         asked_sym = agent_action["request_slots"]
    #         if asked_sym not in self.all_fslots[master_action_index]:
    #             f_reward = f_reward - 1
    #             print('penalized for taking an ambig symmp:', asked_sym)
    #     return f_reward



    def next_by_hrl_joint2(self, dialogue_status, lower_action, state, master_action_index, group_id, episode_over, reward,ar):
        '''
                    self.acc_by_group[group_id][2] += 1
                    if self.master_action_space[master_action_index] == group_id:
                        self.acc_by_group[group_id][1] += 1
        ...
        if lower_reward>0:
            if ar<20:
                lower_reward = lower_reward + 20
        lower_reward = lower_reward + (ar)
    ...
    # def featured_state_representation(self, state):
    #     state_rep = [0]*len(self.fslot_set)
    ...
    #             if value == True and slot in self.fslot_set.keys():
    #                 assert slot in self.fslot_set.keys()
    #                 state_rep[self.fslot_set[slot]] = 1
    #     return state_rep
    ...
    def train_deep_learning_classifier(self, epochs):
        ...
        R = {epochs:{'Diease Classifier Accuracy':test_acc}}
