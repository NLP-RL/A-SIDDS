...
class DialogueManager_HRL(object):
    ...
    def next(self, greedy_strategy, save_record, index):
        ...
        if self.parameter.get("agent_id") == "agenthrljoint2" and self.parameter.get("disease_as_action")==False and agent_action["action"]!="inform":
            ...
            reward, lower_reward = self.next_by_hrl_joint2(dialogue_status, lower_action, state, master_action_index, group_id, episode_over, reward,rr)
    ...
    #         print('Similarity of',current_request, 'with', informs[i] ,ass)
    #         sas = sas + ass
    #     print('Overall Similarity: ', sas)
    #     return sas
    ...
    def next_by_hrl_joint2(self, dialogue_status, lower_action, state, master_action_index, group_id, episode_over, reward,rr):
        '''
                    self.acc_by_group[group_id][2] += 1
                    if self.master_action_space[master_action_index] == group_id:
                        self.acc_by_group[group_id][1] += 1
