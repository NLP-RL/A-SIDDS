...
class DialogueManager_HRL(object):
    ...
    def next(self, greedy_strategy, save_record, index):
        ...
        for key,value in csr.items():
            if value==True:
                t[key]=True
        csrr = t
        #self.pr = self.recommend(csr, csrr)
        if self.parameter.get("agent_id") == 'agentdqn':
        ...
        if master_action_index!=9:
            ...
            ar = self.association(css, agent_action["request_slots"])
        ...
        if self.parameter.get("agent_id") == "agenthrljoint2" and self.parameter.get("disease_as_action")==False and agent_action["action"]!="inform":
            ...
            reward, lower_reward = self.next_by_hrl_joint2(dialogue_status, lower_action, state, master_action_index, group_id, episode_over, reward,rr,ar)
    ...
    def next_by_hrl_joint2(self, dialogue_status, lower_action, state, master_action_index, group_id, episode_over, reward,rr,ar):
        '''
                    self.acc_by_group[group_id][2] += 1
                    if self.master_action_space[master_action_index] == group_id:
                        self.acc_by_group[group_id][1] += 1
