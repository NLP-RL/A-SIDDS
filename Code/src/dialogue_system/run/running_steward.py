...
class RunningSteward(object):
    ...
    def simulation_epoch(self, epoch_size, index):
        ...
        for epoch_index in range(0,epoch_size, 1):
            ...
            while episode_over is False:
                reward, episode_over, dialogue_status,slots_proportion_list,RS= self.dialogue_manager.next(greedy_strategy=True, save_record=True, index=index)
                total_reward += reward
        ...
        res = {"success_rate":success_rate, "average_reward": average_reward, "average_turn": average_turn,
               "average_wrong_disease":average_wrong_disease,"ab_success_rate":absolute_success_rate,'rps':RS[0],'rsc':RS[1]}
    ...
    def evaluate_model(self, dataset, index):
        ...
        # For group-wise performance calculation
        old_repeated_count = 0
        group_wise_performance = {}
        for i in ['12', '13', '14', '19', '1', '4', '5', '6', '7']:
            temp = {"success_rate":0, "average_reward": 0, "average_turn": 0, "average_repeated_action": 0, "average_match_rate2": 0, "average_wrong_disease":0, "average_match_rate":0}
            temp["activation_times"] = 0
            temp["total_reward"] = 0
            temp["num_of_true_slots"] = 0
            temp["num_of_implicit_slots"] = 0
            temp["real_implicit_slots"] = 0
            temp["total_turns"] = 0
            temp["success_count"] = 0
            temp["absolute_success_count"] = 0
            temp["repeated_count"] = 0
            group_wise_performance[i] = copy.deepcopy(temp)
        ...
        for goal_index in evaluate_session_index:
            ...
            # For group-wise performance calculation
            group_id_of_goal = self.dialogue_manager.state_tracker.user.goal['group_id']
            group_wise_performance[group_id_of_goal]["activation_times"] += 1

            while episode_over == False:
                reward, episode_over, dialogue_status,slots_proportion_list,RS = self.dialogue_manager.next(
                    save_record=False,greedy_strategy=False, index=index)
                total_reward += reward

                # For group-wise performance calculation
                group_wise_performance[group_id_of_goal]["total_reward"] += reward
            ...
            # For group-wise performance calculation
            group_wise_performance[group_id_of_goal]["num_of_true_slots"] += slots_proportion_list[0]
            group_wise_performance[group_id_of_goal]["num_of_implicit_slots"] += slots_proportion_list[1]
            group_wise_performance[group_id_of_goal]["real_implicit_slots"] += slots_proportion_list[2]
            ...
            # For group-wise performance calculation
            group_wise_performance[group_id_of_goal]["total_turns"] += self.dialogue_manager.state_tracker.turn

            inform_wrong_disease_count += self.dialogue_manager.inform_wrong_disease_count
            if dialogue_status == dialogue_configuration.DIALOGUE_STATUS_SUCCESS:
                success_count += 1

                # For group-wise performance
                group_wise_performance[group_id_of_goal]["success_count"] += 1

                if self.dialogue_manager.inform_wrong_disease_count == 0:
                    absolute_success_count += 1

                    # For group-wise performance calculation
                    group_wise_performance[group_id_of_goal]["absolute_success_count"] += 1
        ...
        for i in group_wise_performance.values():
            total_times = i["activation_times"]
            if (total_times):
                i["success_rate"]=float(i["success_count"]) / total_times
                i["average_repeated_action"] = float(i["repeated_count"]) / total_times
                i["average_reward"]=float(i["total_reward"]) / total_times
                i["average_turn"] = float(i["total_turns"]) / total_times

            if (i["num_of_implicit_slots"]):
                i["average_match_rate"] = float(i["num_of_true_slots"])/float(i["num_of_implicit_slots"])
            #else:
                #i["average_match_rate"] = 0
            if (i["real_implicit_slots"]):
                i["average_match_rate2"] = float(i["num_of_true_slots"]) / float(i["real_implicit_slots"])
            #else:
                #i["average_match_rate2"] = 0


        pickle.dump(group_wise_performance, open('Grpwise/' + str(index) + '_group_wise_performance.p', 'wb'))
        print("##### group wise performance was saved ###")
        ...
        if self.parameter.get("use_all_labels") == True and self.parameter.get("agent_id").lower() in ["agenthrljoint", "agenthrljoint2"] and self.parameter.get('train_mode') == False:
            pickle.dump(self.dialogue_manager.disease_record, open('disease_record.p', 'wb'), protocol = 2)
            pickle.dump(self.dialogue_manager.lower_reward_by_group, open('lower_reward_by_group.p', 'wb'), protocol = 2)
            pickle.dump(self.dialogue_manager.master_index_by_group, open('master_index_by_group.p', 'wb'), protocol = 2)
            pickle.dump(self.dialogue_manager.symptom_by_group, open('symptom_by_group.p', 'wb'), protocol = 2)
    ...
    def __dump_performance__(self, epoch_index):
        ...
        for i in range(0,len(S)):
            Succ = Succ + [S[i]['success_rate']]
            L = L + [S[i]['average_turn']]
            R = R + [S[i]['average_reward']]
            AMR = AMR + [S[i]['average_match_rate']]
            AMR2 = AMR2 + [S[i]['average_match_rate2']]
        metrics = ['Succ','L','R','AMR','AMR2']
        dat = {'Succ':Succ,'L':L,'R':R,'AMR':AMR,'AMR2':AMR2}
        for j in metrics:
            pyplot.figure()
            pyplot.plot(dat[j])
            pyplot.ylabel(j)
            pyplot.xlabel('Episodes')
            nn = self.checkpoint_path + '/' + j + 'jpg'
            pyplot.savefig(nn)
