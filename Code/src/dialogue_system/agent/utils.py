...
def state_to_representation_history(state, action_set, slot_set, disease_symptom, max_turn):
    ...
    for index in range(0, len(history), 1):
        ...
        rf = state['rf']
        asc= state['ass_score']
        #imp_Symp = state['imp_symp']
        #reward_status = state['reward_status']




        state_rep.append(np.hstack((action_rep,rf,asc, request_rep, inform_slots_rep, explicit_inform_slots_rep,
...
def state_to_representation_last(state, action_set, slot_set, disease_symptom, max_turn):
    ...
    rf = state['rf']
    asc= state['ass_score']
    state_rep = np.hstack((current_slots_rep,rf,asc, user_action_rep, user_inform_slots_rep, user_request_slots_rep, agent_action_rep, agent_inform_slots_rep, agent_request_slots_rep, turn_rep))
    return state_rep
...
def reduced_state_to_representation_last(state, slot_set, parameter):
    ...
    state_rep = np.hstack([c,rf,asc,turn_rep]).flatten()
    # Turn rep.
    #############
    #turn_rep = state["turn"]
    ###Imp symptom
